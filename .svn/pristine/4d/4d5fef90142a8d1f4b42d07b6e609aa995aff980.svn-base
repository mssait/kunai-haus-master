...
export default function GovernmentDetails() {
  ...
  useEffect(() => {
    fetcher('/api/companies/government/my')
      .then(r => r.json())
      .then(setValues)
  }, [])
    ...
    <Formik
      validationSchema={
        Yup.object().shape({
          ...
          population: Yup.number().required('Enter Population'),
      ...
      onSubmit={async (values, { setSubmitting }) => {
        return fetcher('/api/companies/government/my', {
          method: 'put',
      ...
      {({
        ...
        <form noValidate onSubmit={handleSubmit}>
          <Grid container spacing={2}>
            ...
            <Grid item md={4} xs={12}>
              <FormControl fullWidth error={Boolean(touched.population && errors.population)}>
                <TextField
                  required
                  id="population"
                  type="number"
                ...
                {touched.population && errors.population && (
                  <FormHelperText error id="error-population">
                    {errors.population}
                  </FormHelperText>
                )}
              </FormControl>
